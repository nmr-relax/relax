...
def calc_iso_jw_s2(i, frq_index, data):
	...
	The data structure data.mf_index should be an array of integers which point the model-free parameters to the relevant positions in data.params
	It has the following form:
		data.mf_index = [s2_index]
	"""
...
def calc_iso_jw_s2_te(i, frq_index, data):
	...
	The data structure data.mf_index should be an array of integers which point the model-free parameters to the relevant positions in data.params
	It has the following form:
		data.mf_index = [s2_index, te_index]
	"""

	return 0.4 * (data.s2_tm / (1.0 + data.omega_tm_sqrd[i, frq_index]) + (1.0 - data.params[data.mf_index[0]]) * data.te_prime / (1.0 + data.omega_te_prime_sqrd[i, frq_index]))
...
def calc_iso_jw_s2_te_data(data):
	...
	data.te_prime = data.data.params[data.mf_indecies[1]] * data.fact_a
...
def calc_iso_jw_s2f_s2s_ts(i, frq_index, data):
	...
	The data structure data.mf_index should be an array of integers which point the model-free parameters to the relevant positions in data.params
	It has the following form:
		data.mf_index = [s2f_index, s2s_index, ts_index]
	"""

	return 0.4 * data.params[data.mf_index[0]] * (data.s2s_tm / (1.0 + data.omega_tm_sqrd[i, frq_index]) + (1.0 - data.params[data.mf_index[1]]) * data.ts_prime / (1.0 + data.omega_ts_prime_sqrd[i, frq_index]))
...
def calc_iso_jw_s2f_s2s_ts_data(data):
	...
	data.s2 = data.params[data.mf_index[0]] * data.params[mf_index[1]]
	data.fact_a = data.diff_params[0] / (data.params[mf_index[2]] + data.diff_params[0])
	data.ts_prime = data.params[mf_index[2]] * data.fact_a
	data.ts_prime_sqrd = data.ts_prime ** 2
	data.s2s_tm = data.params[mf_index[1]] * data.diff_params[0]
...
def calc_iso_jw_s2f_tf_s2s_ts(i, frq_index, data):
	...
	The data structure data.mf_index should be an array of integers which point the model-free parameters to the relevant positions in data.params
	It has the following form:
		data.mf_index = [s2f_index, tf_index, s2s_index, ts_index]
	"""

	a = data.s2_tm / (1.0 + data.omega_tm_sqrd[i, frq_index])
	b = (1.0 - data.params[data.mf_index[0]]) * data.tf_prime / (1.0 + data.omega_tf_prime_sqrd[i, frq_index]))
	c =  data.params[data.mf_index[0]] * (1.0 - data.params[data.mf_index[2]]) * data.ts_prime / (1.0 + data.omega_ts_prime_sqrd[i, frq_index]))
	return 0.4 * (a + b + c)
...
def calc_iso_jw_s2f_tf_s2s_ts_data(data):
	...
	data.s2 = data.params[data.mf_index[0]] * data.params[mf_index[2]]
	data.fact_f = data.diff_params[0] / (data.params[mf_index[1]] + data.diff_params[0])
	data.fact_s = data.diff_params[0] / (data.params[mf_index[3]] + data.diff_params[0])
	data.tf_prime = data.params[mf_index[1]] * data.fact_f
	data.ts_prime = data.params[mf_index[3]] * data.fact_s
	data.tf_prime_sqrd = data.tf_prime ** 2
	data.ts_prime_sqrd = data.ts_prime ** 2
	data.s2s_tm = data.params[mf_index[2]] * data.diff_params[0]
