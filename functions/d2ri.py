...
def calc_d2noe(data, i, frq_num, get_d2r1):
	...
	for j in range(len(data.params)):
		for k in range(j + 1):
			data.d2r1[i, j, k] = get_d2r1[i](data, i, frq_num, j, k)
			if data.r1[i] == 0.0:
				data.d2ri[i, j, k] = 1e99
			else:
				a = data.ri_prime[i] * (2.0 * data.dr1[i, j] * data.dr1[i, k] - data.r1[i] * data.d2r1[i, j, k])
				b = data.r1[i] * (data.dri_prime[i, j] * data.dr1[i, k] + data.dr1[i, j] * data.dri_prime[i, k] - data.r1[i] * data.d2ri_prime[i, j, k])
				data.d2ri[i, j, k] = data.g_ratio * (1.0 / data.r1[i]**3) * (a - b)

			# Make the hessian symmetric.
			if j != k:
				data.d2r1[i, k, j] = data.d2r1[i, j, k]
				data.d2ri[i, k, j] = data.d2ri[i, j, k]
...
def extract_d2r1(data, i, frq_num, j, k):
	"Get the dR1 value from data.dri_prime"

	return data.d2ri_prime[data.noe_r1_table[i], j, k]
