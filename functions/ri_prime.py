		...
		for i in range(self.mf.data.num_ri):
			...
			# R2 components.
				...
				if match('m3', self.data.mf_model):
					self.data.rex_comps[i] = self.data.mf_params[1] * (1e-8 * self.mf.data.frq[frq_num])**2
				elif match('m4', self.data.mf_model):
					self.data.rex_comps[i] = self.data.mf_params[2] * (1e-8 * self.mf.data.frq[frq_num])**2
