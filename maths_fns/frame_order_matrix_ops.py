...
def compile_2nd_matrix_pseudo_ellipse(matrix, R, eigen_alpha, eigen_beta, eigen_gamma, theta_x, theta_y, sigma_max):
    ...
    # Off diagonal set 1.
    matrix[0, 4] = fact * quad(part_int_daeg2_pseudo_ellipse_04, -pi, pi, args=(theta_x, theta_y, sigma_max), full_output=1)[0]
    matrix[4, 0] = fact * quad(part_int_daeg2_pseudo_ellipse_40, -pi, pi, args=(theta_x, theta_y, sigma_max), full_output=1)[0]
    matrix[0, 8] = fact * quad(part_int_daeg2_pseudo_ellipse_08, -pi, pi, args=(theta_x, theta_y, sigma_max), full_output=1)[0]
    matrix[8, 0] = fact * quad(part_int_daeg2_pseudo_ellipse_80, -pi, pi, args=(theta_x, theta_y, sigma_max), full_output=1)[0]
    matrix[4, 8] = fact * quad(part_int_daeg2_pseudo_ellipse_48, -pi, pi, args=(theta_x, theta_y, sigma_max), full_output=1)[0]
    matrix[8, 4] = fact * quad(part_int_daeg2_pseudo_ellipse_84, -pi, pi, args=(theta_x, theta_y, sigma_max), full_output=1)[0]
...
def populate_2nd_eigenframe_iso_cone(matrix, tmax, smax):
    ...
    matrix[1, 1] = -((sin_2smax*cos_tmax + 3.0*sin_2smax + 12.0*smax)*sin_2tmax2 - 4.0*sin_2smax*cos_tmax + 4.0*sin_2smax) / demon_a
    ...
    # Off diagonal set 1.
    matrix[0, 4] = matrix[4, 0] = (((sin_2smax - 4.0*smax)*cos_tmax + 3.0*sin_2smax)*sin_2tmax2 + (16.0*smax - 4.0*sin_2smax)*cos_tmax + 4.0*sin_2smax - 16.0*smax) / demom_a
    matrix[0, 8] = matrix[8, 0] = -(cos_tmax2 + cos_tmax - 2.0) / 6.0
    matrix[4, 8] = matrix[8, 4] = matrix[0, 8]

    # Off diagonal set 2.
    matrix[1, 3] = matrix[3, 1] = -((sin_2smax*cos_tmax + 3.0*sin_2smax - 12.0*smax)*sin_2tmax2 - 4.0*sin_2smax*cos_tmax + 4.0*sin_2smax) / denom_a
