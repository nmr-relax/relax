...
from maths_fns.frame_order_matrix_ops import compile_2nd_matrix_free_rotor, compile_2nd_matrix_iso_cone, compile_2nd_matrix_iso_cone_free_rotor, compile_2nd_matrix_iso_cone_torsionless, compile_2nd_matrix_pseudo_ellipse, compile_2nd_matrix_pseudo_ellipse_free_rotor, compile_2nd_matrix_pseudo_ellipse_torsionless, compile_2nd_matrix_rotor, reduce_alignment_tensor, pcs_numeric_int_iso_cone, pcs_numeric_int_iso_cone_mcint, pcs_numeric_int_pseudo_ellipse, pcs_numeric_int_pseudo_ellipse_mcint, pcs_numeric_int_rotor, pcs_numeric_int_rotor_mcint
...
class Frame_order:
    ...
    def __init__(self, model=None, init_params=None, full_tensors=None, full_in_ref_frame=None, rdcs=None, rdc_errors=None, rdc_weights=None, rdc_vect=None, rdc_const=None, pcs=None, pcs_errors=None, pcs_weights=None, pcs_atoms=None, temp=None, frq=None, paramag_centre=None, scaling_matrix=None, mcint_num=500, pivot=None, pivot_opt=False, mcint=True):
        ...
        @type pivot_opt:            bool
        @keyword mcint:             A flag which if True will activate the rough Monte Carlo numerical integration rather than the scipy.integrate quad(), dblquad() and tplquad() integration methods.
        @type mcint:                bool
        """
            ...
            if self.pcs_flag:
                ...
                for j in xrange(self.num_pcs):
                    ...
                    if not self.missing_pcs[i, j]:
                        ...
                        self.pcs_theta[i, j] = pcs_numeric_int_rotor(sigma_max=pi, c=self.pcs_const[i], r_pivot_atom=r_pivot_atom, r_ln_pivot=self.r_ln_pivot, A=self.A_3D[i], R_eigen=self.R_eigen, RT_eigen=RT_eigen, Ri_prime=self.Ri_prime)
            ...
            if self.pcs_flag:
                ...
                for j in xrange(self.num_pcs):
                    ...
                    if not self.missing_pcs[i, j]:
                        ...
                        # The numerical integration.
                        self.pcs_theta[i, j] = pcs_numeric_int_pseudo_ellipse(theta_x=cone_theta_x, theta_y=cone_theta_y, sigma_max=cone_sigma_max, c=self.pcs_const[i], r_pivot_atom=r_pivot_atom, r_ln_pivot=self.r_ln_pivot, A=self.A_3D[i], R_eigen=self.R_eigen, RT_eigen=RT_eigen, Ri_prime=self.Ri_prime)
