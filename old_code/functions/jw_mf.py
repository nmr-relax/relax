...
class Jw:
	...
	def initialise_jw_values(self):
		...
		if match('m[13]', self.data.model):
			...
			self.data.s2_tm = self.data.s2 * self.data.tm
			self.data.omega_te_prime_sqrd = zeros((self.relax.data.num_frq, 5), Float64)
			...
			self.data.omega_ts_prime_sqrd = zeros((self.relax.data.num_frq, 5), Float64)
		...
		for i in range(self.relax.data.num_frq):
			for frq_index in range(5):
				...
				if match('m[24]', self.data.model):
					self.data.omega_te_prime_sqrd[i, frq_index] = self.relax.data.frq_sqrd_list[i][frq_index] * self.data.te_prime_sqrd
				elif match('m5', self.data.model):
					self.data.omega_ts_prime_sqrd[i, frq_index] = self.relax.data.frq_sqrd_list[i][frq_index] * self.data.ts_prime_sqrd
