...
uf.desc[-1].add_paragraph("This will copy the diffusion tensor data between data pipes.  The destination data pipe must not contain any diffusion tensor data.  If the pipe_from or pipe_to arguments are not supplied, then both will default to the current data pipe (hence giving one argument is essential).")
# Prompt examples.
...
uf.desc[-1].add_paragraph("The 'time_scale' argument should be a floating point number.  The only parameter affected by this value is tm.")
uf.desc[-1].add_paragraph("The 'd_scale' argument should also be a floating point number.  Parameters affected by this value are Diso, Dpar, Dper, Da, Dx, Dy, and Dz.  Significantly, Dr is not affected.")
uf.desc[-1].add_paragraph("The 'angle_units' argument should either be the string 'deg' or 'rad'.  Parameters affected are theta, phi, alpha, beta, and gamma.")
# Prompt examples.
uf.desc.append(Desc_container("Prompt examples"))
