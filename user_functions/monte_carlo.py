...
uf.add_keyarg(
    ...
    wiz_combo_choices = ["Monte Carlo", "Bootstrapping", "STD from sums of squares"],
    wiz_combo_data = ["back_calc", "direct", "sum_squares"],
    wiz_read_only = True
)
# Description.
uf.desc.append(Desc_container())
uf.desc[-1].add_paragraph("The method can either be set to back calculation (Monte Carlo) or direct (bootstrapping) or sums of squares (STD from sums of squares), the choice of which determines the simulation type.  If the values or parameters are calculated rather than minimised, this option will have no effect.  Errors should only be propagated via Monte Carlo simulations if errors have been measured.  'STD from sums of squares' is where errors are drawn from a distribution of sums of squares of the residual.")
