...
#		"optimize".
#	Stage 3:   Optional use of hand selected models. The file "hand.selection" will be read,
...
#	Stage 4:   Extraction of optimized data.
#
#
# Load parameters:
...
if ( $num_data_sets > 3 ) {                         # ie degrees of freedom > 0 in all models.
	push (@models, "f-m2m4", "f-m3m4", "f-m2m5");
}
if ( $stage == 1 ) {
	print "\n[ File Creation ]\n";
	for $model ( @models ) {
...
if ( $stage == 2 || $stage == 3 ) {
	...
	print "Finished\n\n";
	&close_mfiles;                              # Close the model specific files.
	&close_stage_files;                         # Close the files used in stage 2, 3, and 4.
	if ( $stage == 3 ) {
		&close_stage3_files;                # Close the files used in stage 3.
	}
	print "\n[ End of Stage 2 ]\n\n";
}
if ( $stage == 4 ) {
	&open_stage4_files;                         # Open the files used in stage 4.
...
sub ask_stage {
	...
	print "\tStage 3:   Use of hand selected models (optional).\n";
	print "\tStage 4:   Extraction of optimized data.\n\n";
	$stage = 0;
	until ( $stage == 1 || $stage == 2 || $stage == 3 || $stage == 4 ) {
		local $^W = 0;           # Turn off the warning for undef in the if loop.
		print "Chose the stage by inputting the number 1, 2, 3, or 4.\n";
		print "=>   ";
		chop($stage = <STDIN>);
...
sub open_stage3_files {
	...
	if ( -e "optimize" ) {
		system("mv -f optimize optimize.old");
	}
	system("mkdir optimize");
	if ( -e "grace" ) {
		print "Directory ./grace already exists.\n";
	...
	open(DLOG, ">data.str.stage3");
	open(RESULTS, ">results.stage3");
	open(S2AGR, ">grace/S2.stage3.agr");
	open(S2FAGR, ">grace/S2f.stage3.agr");
	open(TEAGR, ">grace/te.stage3.agr");
	open(REXAGR, ">grace/Rex.stage3.agr");
	open(SSEAGR, ">grace/SSE.stage3.agr");
}

# Open the files used in stage 4.
sub open_stage4_files {
	if ( -e "optimize/mfout" ) {
...
sub ftest {
	...
	while ( <MFOUT> ) {                    # Loop until the end of the SSE data.
		...
		if ( $ftest{$model}[$res]{fstat} > $ftest{$model}[$res]{fstatlim}
			&& $ftest{$model}[$res]{fstat} > 0.5 ) {
...
sub print_data {
	for $res ( 0 .. $total_res ) {              # Loop for all residues.
		...
		foreach $n ( 0 .. $#input ) {
			($NMR_frq_label, $data_type) = split(' ',$input[$n]);
			printf DLOG "%13s%7s%10s", "$NMR_frq_label MHz $data_type", $data_type . "fit", $data_type . "sse";
		}
		printf DLOG "\n";
		for $model ( @models ) {            # Loop for the 5 mfout files.
...
# Close the files used in stage 2, 3, and 4.
...
# Close the files used in stage 3.
sub close_stage3_files {
	close(HAND);
}
