...
from numpy import array, absolute, asarray, diag, dot, exp, eye, log, newaxis, multiply, ones, sqrt, sum, transpose, zeros
...
from specific_analyses.relax_disp.data import average_intensity, generate_r20_key, is_r1_optimised, loop_exp_frq_offset, loop_exp_frq_offset_point, loop_time, return_cpmg_frqs, return_offset_data, return_param_key_from_data, return_r1_data, return_r1_err_data, return_r2eff_arrays, return_spin_lock_nu1
from specific_analyses.relax_disp.parameters import assemble_param_vector, disassemble_param_vector, param_num
from specific_analyses.relax_disp.variables import MODEL_CR72, MODEL_R2EFF, MODEL_TSMFK01, PARAMS_R20
...
def estimate_par_err(spin_id=None, epsrel=0.0, verbosity=1):
    """This will estimate parameter errors from the covariance matrix Qxx.  Qxx is calculated from the Jacobian matrix and the optimised parameters.
    ...
    for cur_spin, mol_name, resi, resn, cur_spin_id in spin_loop(selection=spin_id, full_info=True, return_id=True, skip_desel=True):
        ...
        if model not in jac_models:
            raise RelaxError("The Jacobian matrix has not been specified for '%s', and parameter errors cannot be estimated."%(model))
        ...
        ## Make a single function call, to get the Jacobian and the scaling matrix to convert back to relax_units.
        func_jacobian, func_jacobian_scale_mat = tfunc.get_jacobian_scaled(param_vector)
        ...
        for exp_type, frq, offset, ei, mi, oi in loop_exp_frq_offset(return_indices=True):
            ...
            # If param_vector_error are vector, make it to one-dimensional matrix.
            ...
            cur_func_jacobian_scale_mat = func_jacobian_scale_mat[ei, si, mi, oi]

            # Now scale the vector.
            # Take absolute values, since the frequency can be negative. Errors are always positive.
            param_vector_error_scaled = absolute( multiply(param_vector_error, cur_func_jacobian_scale_mat) )
            ...
            for i, param in enumerate(params):
                ...
                if param in PARAMS_R20:
                    # Copy parameter attribute to error attribute, if not in spin Class.
                    if not hasattr(cur_spin, param_err):
                        setattr(cur_spin, param_err, deepcopy(getattr(cur_spin, param)))

                    # Set error.
                    getattr(cur_spin, param_err)[param_key] = deepcopy(param_vector_error_scaled[i])
                    ...
                    # Set error.
                    setattr(cur_spin, param_err, param_vector_error_scaled[i])
