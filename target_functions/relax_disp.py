...
from numpy import all, arctan2, array, cos, divide, dot, float64, int16, isfinite, max, multiply, ones, rollaxis, pi, sin, sum, transpose, version, zeros
...
class Dispersion:
    def __init__(self, model=None, num_params=None, num_spins=None, num_frq=None, exp_types=None, values=None, errors=None, missing=None, frqs=None, frqs_H=None, cpmg_frqs=None, spin_lock_nu1=None, chemical_shifts=None, offset=None, tilt_angles=None, r1=None, relax_times=None, scaling_matrix=None, recalc_tau=True, r1_fit=False):
        ...
        # A flag which if True will initialise the calculation to scale back the jacobian from rad/s to units in relax and store it.
        ...
        # This is for speed, since it minimises the creation of array structures for each function call.
        # Creating new structures is a safety measure, where a minimisation could possible call both
        # the function and the Jacobian, and overwrite each other. Even though they should be the same.
    ...
    def func_CR72_jacobian(self, params):
        """Jacobian function for the the reduced Carver and Richards (1972) 2-site exchange model on all time scales.
        ...
        # Convert dw from ppm to rad/s. Use the out argument, to pass directly to structure.
        multiply( multiply.outer( dw.reshape(1, self.NS), self.nm_no_nd_ones ), self.frqs, out=self.dw_struct_jac )

        # Reshape R20A to per experiment, spin and frequency.
        self.r20a_struct_jac[:] = multiply.outer( R20.reshape(self.NE, self.NS, self.NM), self.no_nd_ones )

        # Get the Jacobian as list of derivatives.
        simple_d_f_d_r20a, simple_d_f_d_pA, simple_d_f_d_dw, simple_d_f_d_kex = simple_r2eff_CR72_jacobian(r20a=self.r20a_struct_jac, pA=pA, dw=self.dw_struct_jac, kex=kex, cpmg_frqs=self.cpmg_frqs)

        # Convert it to normal representation, where each column is the derivative.
        jacobian = transpose(array( [simple_d_f_d_r20a, simple_d_f_d_pA, simple_d_f_d_dw, simple_d_f_d_kex] ) )

        # If get_jacobian is set to True, calculate a scale back matrix for the Jacobian from rad/s to corresponding units in relax, and then store in class.
        ...
        if self.get_jacobian:
                ...
                simple_d_f_d_dw_scale = divide(ones(simple_d_f_d_dw.shape), self.frqs)

                simple_d_f_d_kex_scale = ones(simple_d_f_d_kex.shape)

                # Collect the scaling matrix.
                self.jacobian_scale_mat =  transpose(array( [simple_d_f_d_r20a_scale, simple_d_f_d_pA_scale, simple_d_f_d_dw_scale, simple_d_f_d_kex_scale] ) )
    ...
    def dfunc_TSMFK01(self, params):
        """Target function for the Jacobian of Tollinger et al. (2001) 2-site very-slow exchange model, range of microsecond to second time scale.
        ...
        # Reshape R20A and R20B to per experiment, spin and frequency.
        self.r20a_struct_jac[:] = multiply.outer( R20A.reshape(self.NE, self.NS, self.NM), self.no_nd_ones )

        # Back calculate the R2eff values.
        r2eff_TSMFK01(r20a=self.r20a_struct_jac, dw=self.dw_struct_jac, dw_orig=dw, k_AB=k_AB, tcp=self.tau_cpmg, back_calc=self.back_calc_jac)
        ...
        dchi2_calc = dchi2_rankN(data=self.values, back_calc_vals=self.back_calc_jac, back_calc_grad=jacobian, errors=self.errors)
    ...
    def func_TSMFK01_jacobian(self, params):
        ...
        # Get the Jacobian as list of derivatives.
        d_f_d_r20a , d_f_d_dw, d_f_d_k_AB = r2eff_TSMFK01_jacobian(r20a=self.r20a_struct_jac, dw=self.dw_struct_jac, k_AB=k_AB, tcp=self.tau_cpmg)
        ...
        if self.get_jacobian:
                ...
                # Collect the scaling matrix.
                self.jacobian_scale_mat =  transpose(array( [d_f_d_r20a_scale, d_f_d_dw_scale, d_f_d_k_AB_scale] ) )
    ...
    def get_jacobian_scaled(self, params):
        ...
        @param params:  The vector of parameter values.
        @type params:   numpy rank-1 float array
        @return:        The Jacobian matrix for the current model, scaled back to relax units.
        ...
        # Now return list with first Jacobian, and scaling matrix to convert to units in relax.
