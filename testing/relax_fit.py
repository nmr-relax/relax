...
class Relax_fit:
    ...
    def read_sparky(self, run):
        ...
        self.relax.interpreter._State.load(file='rx.save', dir=sys.path[-1] + '/testing/data/curve_fitting')

        # Create the run.
        self.relax.generic.runs.create(self.run, "mf")

        # Load the Lupin Ap4Aase sequence.
        self.relax.interpreter._Sequence.read(self.run, file="Ap4Aase.seq", dir=sys.path[-1] + "/testing/data")

        # Read the peak heights.
        self.relax.interpreter._Relax_fit.read(self.run, file="T2_ncyc1_ave.list", dir=sys.path[-1] + "/testing/data/curve_fitting", relax_time=0.0176)
