...
value.set(param='cone_sigma_max', val=pi/2.0)
...
label = 'double_rotor_tilt'
frame_order.pdb_model(ave_pos='ave_pos_%s'%label, rep='frame_order_%s'%label, size=45, force=True)

# Create the PDB.
frame_order.simulate(file='simulation_%s.pdb'%label, step_size=10.0, snapshot=10, total=1000, force=True)
