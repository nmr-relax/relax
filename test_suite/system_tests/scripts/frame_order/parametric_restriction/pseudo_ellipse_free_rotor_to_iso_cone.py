# Script for checking the parametric restriction of the pseudo-ellipse to the isotropic cone frame order model.
# Note this is not a parametric restriction, so it is designed to fail.
...
# Load the tensors.
execfile(__main__.install_path + sep+'test_suite'+sep+'system_tests'+sep+'scripts'+sep+'frame_order'+sep+'tensors'+sep+'iso_cone_out_of_frame_theta_z_tensors_beta78.75.py')
...
for i in range(INC):
    ...
    cdp.cone_theta_x   = cdp.cone_theta_x2   = pi / 4.5
    cdp.cone_theta_y   = cdp.cone_theta_y2   = pi / 4.5
...
#state.save("pseudo_ellipse_to_iso_cone", force=True)
