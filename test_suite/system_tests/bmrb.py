...
class Bmrb(SystemTestCase):
    ...
    def data_check(self, old_pipe_name='results', new_pipe_name='new', version=None):
        ...
        blacklist_spin = ['attached_proton', 'fixed', 'nucleus', 'proton_type', 'relax_sim_data', 'select', 'xh_vect'] + ['r_err', 'csa_err'] + ['chi2_sim', 'f_count', 'g_count', 'h_count', 'iter', 'warning'] + ['frq', 'frq_labels', 'noe_r1_table', 'remap_table', 'ri_labels', 'relax_data', 'relax_error'] + ['_spin_index', '_hash', '_interatomic_hashes']
        if version == '3.0':
