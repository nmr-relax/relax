...
TP02       & Analytic & 2     & $\{\Ronerhoprime, \dots, \pA, \dw, \kex\}$        & -                                 & \citet{TrottPalmer02} \\
\bottomrule
...
This is the model for 2-site fast exchange for off-resonance $\Ronerho$-type experiments from \citet{TrottPalmer02}.  It is selected by setting the model to `TP02'.  The equation for the exchange process is:
\begin{equation}
    \Ronerho = \Rone\cos^2(\theta) + \Ronerhoprime\sin^2(\theta) + \frac{\sin^2(\theta)\pA\pB\dw^2\kex}{\kex^2 + \omgea_{beff}^2} .
\end{equation}
