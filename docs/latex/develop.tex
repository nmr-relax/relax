...
Modifications by relax developers should be made to these sources.
...
\subsection{Git mirrors}

The relax developers are expected to develop directly on the relax repository.
The primary repository is located at \relaxHome.
However, for future longevity of relax, this repository is mirrored on multiple free software/open source infrastructures.
As a relax developer, you should set these all as remotes and push all changes to all mirrors.
Firstly rename the default remote name \example{origin}:

\begin{exampleenv}
\$ git remote rename origin \relaxHomeRemoteName
\end{exampleenv}

Then add all remotes not already present:
...
This can be simplified by using a git alias, by adding the following alias to the git configuration file (e.g. \file{.gitconfig})
