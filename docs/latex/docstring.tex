...
\keyword{axial\_type:}  A string, which if supplied with axially symmetric\index{diffusion!spheroid (axially symmetric)} parameters, will restrict the tensor to either being \quotecmd{oblate} or \quotecmd{prolate}. 
...
The diffusion tensor\index{diffusion!tensor} is defined by the vector $\Diff_\Par$.  The angle\index{angles} $\alpha$ describes the bond vector with respect to the diffusion frame while the spherical angles\index{angles} \{$\theta$, $\phi$\} describe the diffusion tensor\index{diffusion!tensor} with respect to the PDB\index{PDB} frame.  $\theta$ is the polar angle\index{angles} and $\phi$ is the azimuthal angle\index{angles} defined between
...
The \quotecmd{axial\_type} argument should be \quotecmd{oblate}, \quotecmd{prolate}, or None.  The argument will be ignored if the diffusion tensor\index{diffusion!tensor} is not axially symmetric.\index{diffusion!spheroid (axially symmetric)}  If \quotecmd{oblate} is given, then the constraint\index{constraint} $\Diff_\Per$ $\ge$ $\Diff_\Par$ is used.  If \quotecmd{prolate} is given, then the constraint\index{constraint} $\Diff_\Per$ $\le$ $\Diff_\Par$ is used.  If nothing is supplied, then $\Diff_\Per$ and $\Diff_\Par$ will be allowed to have any values.  To prevent minimisation\index{minimisation} of diffusion tensor\index{diffusion!tensor} parameters in a space with two minima, it is recommended to specify which tensor to be minimised,\index{minimisation} thereby partitioning the two minima into the two subspaces (the partition is where $\Diff_a$ equals 0).
...
Within the PDB\index{PDB} frame, the bond vector is described using the spherical angels $\theta$ and $\phi$ where $\theta$ is the polar angle\index{angles} and $\phi$ is the azimuthal angle\index{angles} defined between
...
The \quotecmd{time\_scale} argument should be a floating point\index{floating point number} number.  Parameters affected by this value are:  $\tau_m$.


The \quotecmd{d\_scale} argument should also be a floating point\index{floating point number} number.  Parameters affected by this value are:  $\Diff_{iso}$; $\Diff_\Par$; $\Diff_\Per$; $\Diff_a$; $\Diff_r$; $\Diff_x$; $\Diff_y$; $\Diff_z$.


The \quotecmd{angle\_units} argument should either be the string \quotecmd{deg} or \quotecmd{rad}.  Parameters affected are:  $\theta$; $\phi$; $\alpha$; $\beta$; $\gamma$.
...
To select an axially symmetric\index{diffusion!spheroid (axially symmetric)} diffusion tensor\index{diffusion!tensor} with a $\Diff_\Par$ value of 1.698e7, $\Diff_\Per$ value of 1.417e7, $\theta$ value of 67.174 degrees, and $\phi$ value of -83.718 degrees, and assign it to the run \quotecmd{axial}, type one of:
...
To select and minimise\index{minimisation} an isotropic\index{diffusion!sphere (isotropic)} diffusion tensor,\index{diffusion!tensor} type (followed by a minimisation\index{minimisation} command):
...
\keyword{func\_to$l$:}  The function tolerance value.  Once the function value between iterations decreases below this value, minimisation\index{minimisation} is terminated. 

\keyword{grad\_to$l$:}  The gradient tolerance value. 
...
\begin{itemize}
\item[] Rex is the chemical exchange\index{chemical exchange} value for the current frequency. 
