 ...
 Function for selecting which relaxation\index{relaxation} data to use in the consistency tests. 
 ...
 This function will select the relaxation\index{relaxation} data to use in the consistency tests corresponding to the given frequencies. 
  

  
 \subsubsection{Examples} 

 \example{relax> consistency\_tests.set\_frq(`test', 600.0 * 1e6)} 

 \example{relax> consistency\_tests.set\_frq(run=`test', frq=600.0 * 1e6)} 
 ...
 \example{relax> diffusion\_tensor(`m8', (8.5e-9, 1.1, 20.0, 20.0), param\_types=2)} 
 ...
 \example{relax> diffusion\_tensor(`spheroid', (1.698e7, 1.417e7, 67.174, -83.718), param\_types=3)} 

 \example{relax> diffusion\_tensor(run=`spheroid', params=(1.698e7, 1.417e7, 67.174, -83.718), param\_types=3)} 

 \example{relax> diffusion\_tensor(`spheroid', (1.698e-1, 1.417e-1, 67.174, -83.718), param\_types=3, d\_scale=1e8)} 

 \example{relax> diffusion\_tensor(run=`spheroid', params=(1.698e-1, 1.417e-1, 67.174, -83.718), param\_types=3, d\_scale=1e8)} 

 \example{relax> diffusion\_tensor(`spheroid', (1.698e-1, 1.417e-1, 1.1724, -1.4612), param\_types=3, d\_scale=1e8, angle\_units=`rad')} 

 \example{relax> diffusion\_tensor(run=`spheroid', params=(1.698e-1, 1.417e-1, 1.1724, -1.4612), param\_types=3, d\_scale=1e8, angle\_units=`rad', fixed=1)} 
 ...
 \textsf{\textbf{grace.write}(self, run=None, x\_data\_type=`res', y\_data\_type=None, res\_num=None, res\_name=None, plot\_data=`value', norm=0, file=None, dir=`grace', force=0)} 
 ...
 \keyword{ norm:}  Flag for the normalisation of series type data.   
 ...
 By plotting\index{plot} the peak intensities, the integrity of exponential relaxation\index{relaxation} curves can be checked and anomalies searched for prior to model-free analysis or reduced spectral density mapping.\index{map}  For example the normalised average peak intensities can be plotted\index{plot} verses the relaxation\index{relaxation} time periods for the relaxation\index{relaxation} curves of all residues of a protein.  The normalisation, whereby the initial peak intensity of each residue I(0) is set to 1, emphasises any problems.  To produce this Grace\index{computer programs!Grace}\index{computer programs!Grace} file, type: 
  

 \example{relax> grace.write(name, x\_data\_type=`relax\_times', y\_data\_type=`ave\_int', norm=1, file=`intensities\_norm.agr', force=1)} 
 ...
 As both the line search algorithm and Hessian modification can be specified for Newton\index{minimisation techniques!Newton} optimisation, supplying the option \quotecmd{None} may appear to be ambiguous.  In fact, the Hessian takes precedence over the line search.  Hence supplying the option \quotecmd{None} with the Newton\index{minimisation techniques!Newton} algorithm selects the unmodified Hessian.  Supplying \quotecmd{None} twice will turn off the Hessian modification and no step length selection (or line search) will be performed. 
 ...
 \begin{description} 
 \item[\quotecmd{ct} --]  Consistency tests.  
 ...
 In consistency testing, only four values can be set, the bond\index{bond length} length, CSA, angle\index{angles} Theta (\quotecmd{orientation}) and correlation\index{correlation time} time values. These must be set prior to the calculation of consistency functions. 
 ...
 To set the CSA value of all residues in the reduced spectral density mapping\index{map} run \quotecmd{600MHz} to -172 ppm, type: 
  

 \example{relax> value.set(`600MHz', -172 * 1e-6, `csa')} 

 \example{relax> value.set(`600MHz', value=-172 * 1e-6, param=`csa')} 
