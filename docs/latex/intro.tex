...
relax has been designed such that knowledge about Python \index{Python} is not required to be able to fully use the program.  A few basics, though, will aid in understanding relax.

A number of simple programming axioms includes that of strings \index{string}, integers \index{integer}, floating point numbers \index{floating point number}, and lists \index{list}.  A string is text and within Python \index{Python} (as well as relax) this is delimited by either single or double quotes.  An integer is a number with no decimal point while a float is a number with a decimal point.  A list in Python \index{Python} (called an array in other languages) is a list of anything separated by commas and delimited by square brackets, an example is [0, 1, 2, `a', 1.2143235].

Probably the most important detail is that functions in Python \index{Python} require brackets around their arguments.  For example
...
A few sample scripts \index{sample scripts} have been provided in the directory `sample\_scripts'.  These can be used as a good starting point for using relax.
