...
For certain algorithms, such a simplex minimisation\index{optimisation!Nelder-Mead simplex algorithm}, this single value suffices.
...
The most powerful minimisation algorithm for model-free analysis -- Newton optimisation\index{optimisation!Newton algorithm} -- requires the value, gradient, and Hessian at the current parameter values.
...
\index{optimisation!steepest descent algorithm|textbf}
...
\index{optimisation!coordinate descent algorithm|textbf}
...
\index{optimisation!BFGS algorithm|textbf}
...
\index{optimisation!Newton algorithm|textbf}
...
\index{optimisation!Newton-CG algorithm|textbf}
...
\noindent where $m_k(p)$ is the quadratic model, $B_k$ is a positive definite matrix which can be the true Hessian as in the Newton model or an approximation such as the BFGS\index{optimisation!BFGS algorithm} matrix, and $\lVert p \rVert$ is the Euclidean norm of $p$.  The trust region radius $\Delta_k$ is modified dynamically during optimisation -- if the quadratic model is found to be a poor representation of the space the radius is decreased whereas if the quadratic model is found to be reasonable the radius is increased to allow larger, more efficient steps to be taken.


% The Cauchy point algorithm.
\subsubsection{The Cauchy point algorithm}
\index{optimisation!Cauchy point algorithm|textbf}

The Cauchy point algorithm is similar in concept to the steepest descent\index{optimisation!steepest descent algorithm} line search algorithm.  The Cauchy point is the point lying on the gradient which minimises the quadratic model subject to the step being within the trust region.  By iteratively finding the Cauchy point the local minimum can be found.  The convergence of the technique is inefficient, being similar to that of the steepest descent algorithm.


% The dogleg algorithm.
\subsubsection{The dogleg algorithm}
\index{optimisation!dogleg algorithm|textbf}
...
\index{optimisation!CG-Steihaug algorithm|textbf}
...
\index{optimisation!exact trust region algorithm|textbf}
...
\index{optimisation!Nelder-Mead simplex algorithm|textbf}
...
\index{optimisation!Levenberg-Marquardt algorithm|textbf}
...
\index{optimisation!Method of Multipliers constraint algorithm|textbf}
...
\index{optimisation!Logarithmic barrier constraint algorithm|textbf}
